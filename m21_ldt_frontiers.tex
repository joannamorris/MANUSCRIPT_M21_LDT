...
A substantial body of research demonstrates that morphological structure influences visual word recognition. For example, the time it takes to recognize a monomorphemic word (e.g. `bird' or `walk')is influenced not only by its own token frequency but those of all its inflectional variants (e.g. `birds', `walking', `walked', `walks'). Suprisingly, recognition time for monomorphemic words is not influenced by the token frequency of derivational variants and compounds  (e.g. `birdcage', `walker') but rather by their type frequency, i.e. the number of distinct derived words and compounds in which the base words features. 
...
Whether these effects reflect explicit morpheme representations or emergent statistical sensitivity to regularities among word forms remains contested \citep{stevensPlaut2022}, and current methods are unlikely to fully resolve this debate. The present study therefore remains agnostic on this theoretical question, focusing instead on whether the processes underlying morphological sensitivity vary systematically with reader skill — a question that is meaningful under either account.

Yet despite robust evidence that morphological structure shapes word recognition, relatively little is known about whether and how these effects vary across readers. If morphological sensitivity is graded — emerging from learned statistical regularities rather than fixed representational primitives — then individual differences in the depth or efficiency of that learning should modulate the morphological effects described above, and may do so differently depending on the type of linguistic knowledge involved.
